...
import com.example.noteappkhaled.databinding.GridViewMediumBinding
import com.example.noteappkhaled.model.Notes
import java.text.SimpleDateFormat
import java.util.Locale

class MediumGridAdapter : RecyclerView.Adapter<MediumGridAdapter.SmallGirdViewHolder>() {
    var noteList = listOf<Notes>()
    private val dataFormat = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault())
    private val timeFormat = SimpleDateFormat("HH:mm", Locale.getDefault())

    inner class SmallGirdViewHolder(val  binding : GridViewMediumBinding) : RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SmallGirdViewHolder {
        return SmallGirdViewHolder(GridViewMediumBinding.inflate(LayoutInflater.from(parent.context),parent,false))
    }
